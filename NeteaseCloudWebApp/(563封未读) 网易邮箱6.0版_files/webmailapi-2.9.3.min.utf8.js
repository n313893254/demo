...
if (!JSON) {
	...
	function c(o) {
		...
		if(/^([\w\-\_]+\.)+[\w\-]{2,7}$/ig.test(j) && /((163\.com)|(126\.com)|(yeah\.net)|(188\.com))$/ig.test(j)){
			var n = "//" + j + "/app/h/service_exec_func.htm?eval=" + escape(o);
	...
	function d(x, w, j) {
		...
		var s = w || window.name;
		var k = location.protocol + "//" + q + "/app/h/height.htm#" + s + "|" + x;
		if (h.iframe) {
			h.iframe.src = k
		} else {
	...
	function a(j) {
		...
		k.getFolderInfo({
			call: function() {
				var n = m.host;
				var o = m.sid;
				var p = location.protocol + "//" + n + "/js5/main.jsp?sid=" + o;
				if (j) {
					if (j.module) {
						switch (j.module) {
						case "read":
	...
	function a(d) {
		d = d || {};
		var e = this;
		fFlashRequestInitialize("https://mimg.127.net/mailapi/flashRequest.swf", null, null, d.onReady);
...
function _fEval(sValue) {
	...
	function a() {
		return location.protocol + "//" + h.host + "/app/s"
	}
	function m(s, x) {
...
function fFlashRequestInitialize(n, g, a, t) {
	...
	function o() {
		if (FlashRequest.global.hasFlash && !FlashRequest.global.created) {
			...
			if (FlashRequest.global.isIE) {
				A = '<object id="' + FlashRequest.global.id + '" name="' + FlashRequest.global.id + '" width="1" height="1" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="https://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab"><param name="movie" value="' + FlashRequest.global.url + '" /><param name="quality" value="high" /><param name="wmode" value="transparent" /><param name="allowScriptAccess" value="always" /><param name="FlashVars" value="' + w + '" /></object>'
			} else {
	...
	J._$registXMessage = function(_domain, _listener) {
		...
		__cache[_dmn] = {
			div: _ntmp,
			cbs: _listener || "J.g",
			url: _domain.toLowerCase().indexOf(location.protocol + "//") >= 0 ? _domain: location.protocol + "//" + _domain + "/proxy.html"
		};
		return __cache[_dmn]
	}
})(); (function() {
	...
	function fNeteaseMailApiCrossInit(oParam) {
		...
		sOriginHost = location.protocol + "//" + sWmsvrHost;
		if (!oParam.proxy) {
			if (sWmsvrHost) {
				oParam.proxy = location.protocol + "//" + sWmsvrHost + "/app/h/proxy.htm"
			}
		}
		function _() {
			if (oParam.proxy) {
	...
	window.Webmail = window.WebmailHelper = {
		...
		appApi : function fNeteaseMailApiServiceAppApi(oParam){
				...
				if(/^([\w\-\_]+\.)+[\w\-]{2,7}$/ig.test(sHost) && /((163\.com)|(126\.com)|(yeah\.net)|(188\.com))$/ig.test(sHost)){
					var sUrl = location.protocol + "//"+ sHost +"/app/h/service_exec_func.htm?func=" + oParam.func + "&param=" + encodeURIComponent(encodeURIComponent(oParam.param));
